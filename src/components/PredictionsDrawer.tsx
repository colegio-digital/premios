...
export const PredictionsDrawer: React.FC<PredictionsDrawerProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-fade-in">
      <div className="relative w-full max-w-2xl bg-slate-900 border border-red-500/30 rounded-2xl shadow-2xl overflow-hidden max-h-[85vh] flex flex-col">
        
        {/* Header */}
        <div className="flex items-center justify-between p-5 border-b border-slate-800 bg-slate-950/60">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-red-400 to-rose-700 text-slate-950 flex items-center justify-center font-bold">
              <Trophy className="w-5 h-5 text-slate-950" />
            </div>
            <div>
              <h3 className="text-lg font-bold text-white flex items-center gap-2">
                <span className="text-red-100">Mis Predicciones & Favoritos</span>
        ...
        <div className="p-6 overflow-y-auto space-y-3">
          {votedCount === 0 ? (
            <div className="text-center py-12 text-slate-400">
              <Award className="w-12 h-12 mx-auto text-red-500/40 mb-3" />
              <p className="text-sm font-medium text-red-200">Aún no has seleccionado ningún favorito.</p>
            ...
            categories.map((cat) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="shrink-0 p-1.5 rounded-full bg-red-500/10 text-red-400">
          ...
          <div className="flex items-center gap-2 ml-auto">
            {votedCount > 0 && (
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 rounded-xl bg-red-500 hover:bg-red-400 text-slate-950 font-bold text-xs transition-colors shadow-md shadow-red-500/20"
              >
                {copiedSummary ? (
                  <>
                    <Check className="w-4 h-4" />
